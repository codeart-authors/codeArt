...
def create_attention_mask_aggressive(
    ...
    # assert(torch.sum(torch.diagonal(attention_mask, 0)) == 511) # debug
...
class MaskBuilder(object):
    ...
    def create_attention_mask(
        ...
        if self.preset == 'aggressive':
            return create_attention_mask_aggressive(input_length, instruction_node_positions, data_dep)
        elif self.preset == 'conservative':
            return create_attention_mask_conservative(input_length, instruction_node_positions, data_dep)
        elif self.preset == 'graphcodebert':
    ...
    def create_attention_mask_and_relative_position_matrix(
        ...
        # print(torch.max(rel_pos_matrix))
        ...
        # return torch.ones(size=(input_length, input_length), dtype=torch.bool), rel_pos_matrix
